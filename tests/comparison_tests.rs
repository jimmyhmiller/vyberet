...
fn test_pyret_match_long_dot_chain() {
    let expr = format!(
        "obj{}",
        (1..=10).map(|n| format!(".field{}", n)).collect::<String>()
    );
    assert_matches_pyret(&expr);
}
...
fn test_for_with_function_call_iterator() {
    ...
    assert_matches_pyret(
        r#"for make-checker(loc)(lv from [list: 1, 2], rv from [list: 3, 4]):
  lv + rv
end"#,
    );
}
...
fn test_full_file_modules_provide_arrow_using_datatype() {
    let code =
        include_str!("pyret-files/full-files/tests/modules/provide-arrow-using-datatype.arr");
    assert_matches_pyret(code);
}
...
fn test_full_file_modules_test_provide_data_with_provide_star() {
    let code = include_str!(
        "pyret-files/full-files/tests/modules/test-provide-data-with-provide-star.arr"
    );
    assert_matches_pyret(code);
}
